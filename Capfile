...
set :nhosts, 1
set :group_name, 'Mash1_TC_ChIPseq'
...
task :fetch_human_genome, :roles => group_name do
  run "cd #{working_dir}/bowtie-0.12.7/indexes && curl  ftp://ftp.cbcb.umd.edu/pub/data/bowtie_indexes/hg19.ebwt.zip > hg19.ebwt.zip"
  run "cd  #{working_dir}/bowtie-0.12.7/indexes && unzip -o hg19.ebwt.zip"
end 
...
# try bowtie on human genome
task :run_bowtie_vs_human, :roles => group_name do
  ...
  files.each{|infile|
    outfile = infile.sub('.fastq', '.sam')
    run("#{working_dir}/bowtie-0.12.7/bowtie  --sam --best -k1 -l15 -n1 -m3 -p20 --solexa1.3-quals --chunkmbs 256  -q hg19 --quiet #{infile}  > #{outfile} ")
  } 

end
before "run_bowtie_vs_human", "EC2:start"
...
desc "make bam from sam"
task :human_to_bam, :roles => group_name do
